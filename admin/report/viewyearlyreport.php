...
<?php 
		...
		if( ! mysqli_num_rows($result) ){
				...
				echo "<p style='font-size:25px;'><b>There is no data in this date range.</b></p>";
		}
		
		else{
		echo "<p>";
		echo "Date range: ";
		echo "<input readonly value='$start'> to <input readonly value='$end'> ";
		echo "</p>";
		...
		$sql = "SELECT COUNT(Booking_id) AS Number_of_customer FROM booking WHERE Booking_date BETWEEN '$start' AND '$end'";
		$res = mysqli_query($link, $sql);
		echo "<br><table class='b'>";
		echo "<tr><td>Number of customer of the selected date range: ";
		$count = mysqli_fetch_array($res);
		echo "".$count['Number_of_customer']."</td>";
		
		
		$sql2 = "SELECT SUM(Booking_fees) AS Yearly_income FROM booking WHERE Booking_date BETWEEN '$start' AND '$end'";
		$rs = mysqli_query($link, $sql2);
		echo "<tr><td>Income of the selected date range: RM ";
		$count = mysqli_fetch_array($rs);
		echo "".$count['Yearly_income']."</td>";
		}
		echo "</table>";
		}
		mysqli_close($link);
...
<form name="back" action="yearly.html">
<br>
