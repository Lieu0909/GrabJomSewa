...
<?php 
		...
		$data = "SELECT booking.Booking_date,
				...
				WHERE YEAR(Booking_date)='$year' AND MONTH(Booking_date)='$monthly'
		...
		if( ! mysqli_num_rows($result) ){
				...
				echo "<p style='font-size:25px;'><b>There is no data in this month.</b></p>";
		}
		...
		echo "<p>";
		echo "Date: ";
		echo "<input readonly name='month' value='$month'>";
		echo "</p>";
		...
		$sql = "SELECT COUNT(Booking_id) AS Number_of_customer FROM booking WHERE YEAR(Booking_date)='$year' AND MONTH(Booking_date)='$monthly'";
		$res = mysqli_query($link, $sql);
		echo "<br><table class='b'>";
		echo "<tr><td><b>Number of customer of the month: ";
		$count = mysqli_fetch_array($res);
		echo "".$count['Number_of_customer']."</b></td></tr>";
		
		
		$sql2 = "SELECT SUM(Booking_fees) AS Daily_income FROM booking WHERE YEAR(Booking_date)='$year' AND MONTH(Booking_date)='$monthly'";
		$rs = mysqli_query($link, $sql2);
		echo "<tr><td><b>Daily income of the month: RM ";
		$count = mysqli_fetch_array($rs);
		echo "".$count['Daily_income']."<b></td></tr>";
		echo "</table>";
		}
		mysqli_close($link);
...
<form name="back" action="monthly.html">
<br>
